...
const noise = new Noise(225);

function drawBackground() {
  ctx.fillStyle = 'rgb(29, 42, 46)';
...
function getNoiseDelta(time, direction, input_index) {
    let i = input_index/100; 
    let x1 = 0;
    let y1 = 0;
    let x2 = x1 + 24;
    let y2 = y1 + 24;
    ...
    let normaliseFactor = 2;
    // x1 = x1 / normaliseFactor;
    // y1 = y1 / normaliseFactor;
    // x2 = x2 / normaliseFactor;
    // y2 = y2 / normaliseFactor;
    ...
    //   console.log("Noise value for checkpoint " + i + ": ", mynewnoise); 
    // console.log(normalize(x1,y1));
    return {
        dx: Math.abs(noise.perlin2(x1/normaliseFactor, y1/normaliseFactor)) * scale * dir,
        dy: Math.abs(noise.perlin2(x2/normaliseFactor, y2/normaliseFactor)) * scale * -1,
    };
...
function generate_branch(x, y, dir){
    ...
    for (let i = 0; i < checkpoint_count; i++) {
        for (let j = 0; j < checkpoint_spacing_steps; j++) {
            time2 += fixed_time_step;
            const delta = getNoiseDelta(time2, dir, i);
            console.log ("Delta for checkpoint " + i + ": ", delta);
            sim_x += delta.dx;
            sim_y += delta.dy;
        }
        generated.push({x: sim_x, y: sim_y});
